...
import { InfographicVariant } from "@/components/infographics/InfographicVariant";
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          {infographicData && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {(['light-1', 'light-2', 'dark-1', 'dark-2'] as const).map((variant) => (
                <InfographicVariant
                  key={variant}
                  id={`infographic-${variant}`}
                  title={infographicData.title}
                  points={infographicData.points}
                  variant={variant}
                  onDownload={downloadInfographic}
                />
              ))}
            </div>
          )}
